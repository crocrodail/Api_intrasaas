  ...
  for (const key in protectedRoot) {
    if (!protectedRoot[key].role) {
      if (protectedRoot[key].path.includes(":")) {
        unlessPath.push({
          url: new RegExp(protectedRoot[key].path.split(":")[0] + ".*"),
          methods: [protectedRoot[key].method],
        });
      } else {
        unlessPath.push({
          url: protectedRoot[key].path,
          methods: [protectedRoot[key].method],
        });
      }
    }
  ...
  for (const key in protectedRoot) {
    if (protectedRoot[key].method === req.method && protectedRoot[key].role) {
      ...
      if (splitUrl.length > 1) {
        const baseUrls = {
          url: splitUrl[0].replace(/\//g, ""),
          params: splitUrl.length - 1,
        };
        if (
          urlCall[0] === baseUrls.url &&
          baseUrls.params == urlCall.length - 1
        ) {
          if (
            payload.payload.role.some((item) =>
              protectedRoot[key].role.includes(item)
            ) ||
            protectedRoot[key].role.includes("*")
          ) {
            isIn = true;
          }
        }
